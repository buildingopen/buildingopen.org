...
export default function Card({ href, icon, title, description, meta, tags, external, disabled }: CardProps) {
  const content = (
    <>
      ...
      <h3 className="font-semibold mb-1 group-hover:text-white transition-colors">{title}</h3>
  ...
  const className = `group relative block border border-dashed rounded-lg p-5 transition-all duration-300 ${
    disabled
      ...
      : 'border-zinc-800 hover:border-zinc-500/50 hover:-translate-y-1 hover:shadow-xl hover:shadow-zinc-500/5 card-glow'
  }`;
